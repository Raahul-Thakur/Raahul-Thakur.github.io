...
import { Podcast, FileDown, BookOpen, Newspaper, Mail, Menu, X } from 'lucide-react';
      ...
      <header className={`py-6 px-4 md:py-8 md:px-12 border-b-2 border-cyan-400 fixed top-0 left-0 right-0 bg-black/80 backdrop-blur-sm z-50 transition-transform duration-300 ${visible ? 'translate-y-0' : '-translate-y-full'}`}>
        <div className="max-w-6xl mx-auto flex items-center justify-between">
          ...
          <button
            ...
            className="md:hidden p-2 border-2 border-cyan-400 hover:bg-cyan-400 hover:text-black transition-all duration-300"
          >
            {mobileMenuOpen ? (
              <X className="w-6 h-6" />
            ) : (
        ...
        {mobileMenuOpen && (
          <motion.div
            ...
            className="md:hidden mt-4 pt-4 border-t-2 border-cyan-400 space-y-3"
          >
            <button
              onClick={() => {
              ...
              className="w-full flex items-center justify-center gap-3 px-4 py-2 border-2 border-cyan-400 hover:bg-cyan-400 hover:text-black transition-all duration-300"
            >
              Home
            </button>
            <Link
